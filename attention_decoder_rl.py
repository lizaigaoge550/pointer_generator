...
def attention_decoder(_hps,  v_size,  _max_art_oovs,  _enc_batch_extend_vocab,  emb_dec_inputs,  _dec_in_state,
  ...
  initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None, embedding=None, schedule_sampling=None,
    ...
    def intra_decoder_attention(decoder_state, outputs):
        ...
        _prev_decoder_features = nn_ops.conv2d(_decoder_states, W_h_d, [1, 1, 1, 1],
        ...
        with variable_scope.variable_scope("DecoderAttention"):
            if len_dec_states > 0:
                ...
                decoder_features = tf.expand_dims(
                    tf.reshape(decoder_features, [batch_size, -1, attention_dec_vec_size]),
                    axis=1)  # [batch_size, l ,1, dec_hidden]
                e_not_masked = math_ops.reduce_sum(v_d * math_ops.tanh(_prev_decoder_features + decoder_features),
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
        ...
        for i, inp in enumerate(emb_dec_inputs):
            ...
            one_hot_k_samples = tf.distributions.Multinomial(total_count=1., probs=final_dist).sample(1)  # sample once according to https://arxiv.org/pdf/1705.04304.pdf, size (k,batch_size,extended_vsize)
    ...
    return outputs, state, attn_dists, p_gens, coverage, vocab_scores, final_dists, samples, greedy_search_samples
