...
from attention_decoder_v1 import attention_decoder
import numpy as np
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('decoder'):
                ...
                self.greedy_search_samples, self.attn_max = \
...
#_hps,  v_size,  _max_art_oovs,  _enc_batch_extend_vocab,  emb_dec_inputs,  _dec_in_state, _enc_states, enc_padding_mask, cell,
  #initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None, prev_encoder_es=[]
    ...
    def _add_shared_loss_op(self):
        ...
        #self.attn_max is list(dec_step) of batch
        with tf.variable_scope('attn_loss'):
            attn_loss = []
            for attn_step, attn in enumerate(self.attn_max):
                targets = self._target_batch[:, attn_step]
                attn_loss.append(tf.reduce_sum(0.5 * tf.squared_difference(tf.cast(attn,tf.float32),tf.cast(targets,tf.float32)) * self._dec_padding_mask[:,attn_step]))
            self._attn_loss = tf.reduce_mean(tf.stack(attn_loss))
    ...
    def loss_function(self):
        self.loss = self._pgen_loss + self._hps.attn_gamma * self._attn_loss
        if self._hps.coverage:
            self.loss = self._pgen_loss + self._hps.cov_loss_wt * self._coverage_loss + self._hps.attn_gamma * self._attn_loss
