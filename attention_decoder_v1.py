...
#attn_dist _enc_batch_extend_vocab --> (batch_size, ?)
def get_max_attention_word_index(attn_dist, _enc_batch_extend_vocab, _hps):
    argmax = tf.argmax(attn_dist, axis=1) #batch
    batch_nums = tf.range(0, limit=_hps.batch_size) #[batch_size]
    indices = tf.stack((tf.cast(batch_nums,argmax.dtype), argmax),axis=1)
    return tf.gather_nd(_enc_batch_extend_vocab,indices)
  ...
  initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None, embedding=None, schedule_sampling=None,
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
        ...
        for i, inp in enumerate(emb_dec_inputs):
            ...
            attn_max.append(get_max_attention_word_index(attn_dist,_enc_batch_extend_vocab,_hps))
    ...
    return outputs, state, attn_dists, p_gens, coverage, vocab_scores, final_dists, greedy_search_samples, attn_max
