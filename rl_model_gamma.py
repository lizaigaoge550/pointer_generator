...
from attention_decoder_rl import attention_decoder
import numpy as np
...
class RLNet(object):
    ...
    def _add_placeholders(self):
        ...
        self._reward = tf.placeholder(tf.float32, [hps.batch_size,hps.max_dec_steps], name='reward')


        self._eta = tf.placeholder(tf.float32, [], name='eta')
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('decoder'):
                ...
                self.samples, self.greedy_search_samples = \
...
#_hps,  v_size,  _max_art_oovs,  _enc_batch_extend_vocab,  emb_dec_inputs,  _dec_in_state, _enc_states, enc_padding_mask, cell,
  #initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None, prev_encoder_es=[]
    def _add_decoder(self, emb_dec_inputs):
        ...
        prev_decoder_outputs = self.prev_decoder_outputs if (hps.intradecoder and hps.mode == "decode") else tf.stack(
            [], axis=0)
    ...
    def _add_shared_loss_op(self):
        ...
        for dec_step, dist in enumerate(self.final_dists):
            targets = tf.squeeze(self.samples[dec_step])
            indices = tf.stack((batch_nums, targets), axis=1)
            gold_probs = tf.gather_nd(dist, indices)
            losses = -tf.log(gold_probs)
            loss_per_step.append(losses)
            rl_losses = -tf.log(gold_probs)
            rl_loss_per_step.append(rl_losses)
        with tf.variable_scope('reinforce_loss'):
            self._rl_loss = tf.reduce_mean(tf.transpose(tf.squeeze(loss_per_step)) * self._dec_padding_mask * self._reward)
    ...
    def loss_function(self):
        ...
        if self._hps.coverage:
            self.loss = self._eta * (self._pgen_loss + self._hps.cov_loss_wt * self._coverage_loss) + self._rl_loss * (1 - self._eta)
        else:
            self.loss = self._eta * self._pgen_loss + self._rl_loss * (1-self._eta)
